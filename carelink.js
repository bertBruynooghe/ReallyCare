/* Modified copy of https://github.com/mddub/minimed-connect-to-nightscout/blob/master/carelink.js */
/* Too bad it wasn't written as a proper module; I might be doing that one day. */
/* jshint node: true */
"use strict";
...
var logger = {log: function(){ console.log(arguments);}};

var DEFAULT_MAX_RETRY_DURATION = module.exports.defaultMaxRetryDuration = 512;

var CARELINK_SECURITY_URL = process.env.THE_SITE + '/patient/j_security_check';
var CARELINK_AFTER_LOGIN_URL = process.env.THE_SITE + '/patient/main/login.do';
var CARELINK_JSON_BASE_URL = process.env.THE_SITE + '/patient/connect/ConnectViewerServlet?cpSerialNumber=NONE&msgType=last24hours&requestTime=';
...
function responseAsError(response) {
  if (!(response.statusCode >= 200 && response.statusCode < 400)) {
    return new Error(
      ...
      JSON.stringify(_.merge(response, {}))
    );
  } else {
    return null;
  }
